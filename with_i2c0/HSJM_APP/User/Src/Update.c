...
*Function description:从i2c那里接收的数据中查找并识别是否包含目标数组
*Ipunt				 :
*OutPut				 :SUCCSE or ERROR
*-----------------------------------------------------------------------------*/
ErrStatus i2c_receive_check(uint8_t source_arr[20], uint8_t target_arr[11], uint8_t *new_arr) 
{
    for (int i = 0; i <= 20 - 11; ++i) //总共就循环9次，找不到就不用找了 
	{
        int found = 1;
        for (int j = 0; j < 11; ++j) 
		{
            if (source_arr[i + j] != target_arr[j]) //0		0 //i==0说明是第一次找 
			{										//0+1	1
                found = 0;							//0+2	2
                break;								//0+3	3
            }
        ...
        if (found) 									//找到了第一个数 
		{
            ...
            for (int l = 0; l < 11; ++l) 
			{
                if (new_arr[l] != target_arr[l])    //新数组和目标数组检查
				{
                    return ERROR;
                }
            }
...
{
    uint8_t new_arr[11];
	if(i2c_receive_check(Update_tI2cSlave.RecBuff, Std_Receive_Arr.QueryBootloaderStatus, new_arr) == SUCCESS)
